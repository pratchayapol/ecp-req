        ...
        <div class="flex-1 flex flex-col justify-between bg-white/60 mt-6 me-6 mb-6 rounded-[20px] overflow-auto">
            <div class="p-8">
                <div class="bg-white rounded-lg shadow-lg h-auto">
                    ...
                    <div class="flex justify-center items-center min-h-screen text-center px-4">
                        <div class="max-w-3xl">
                            <?php
                            try {
                                ...
                                if ($stmt->rowCount() == 0) {
                                    echo '<h3 class="text-red-600 text-xl font-semibold">!!! ไม่พบข้อมูลการประกาศข่าว !!!</h3>';
                                } else {
                                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                        $ThDay = ["อาทิตย์", "จันทร์", "อังคาร", "พุธ", "พฤหัสบดี", "ศุกร์", "เสาร์"];
                                        $ThMonth = ["มกราคม", "กุมภาพันธ์", "มีนาคม", "เมษายน", "พฤษภาคม", "มิถุนายน", "กรกฏาคม", "สิงหาคม", "กันยายน", "ตุลาคม", "พฤศจิกายน", "ธันวาคม"];

                                        $myDATE = $row['date_published'];
                                        $time = date("H:i:s", strtotime($myDATE));
                                        ...
                                        echo '<h3 class="text-2xl font-bold mb-4">' . htmlspecialchars($row["article_title"]) . '</h3>';
                                        echo '<div class="prose prose-lg mx-auto mb-4">' . $row["article_content"] . '</div>';
                                        echo '<span class="block text-sm text-gray-500">แก้ไขเมื่อ : ' . $datetime . '</span>';
                                    }
                                }
                            } catch (PDOException $e) {
                                echo '<p class="text-red-600">Connection failed: ' . $e->getMessage() . '</p>';
                            }
                            ?>
                        </div>
                    </div>
